...
contract DebtToken is ERC20, AccessControl {
    ...
    constructor() ERC20("Debt Token", "dUSD") {
        // Grant the deployer the admin and minter roles
        _grantRole(ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "DebtToken: caller is not a minter");
        _;
    }

    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "DebtToken: caller is not an admin");
        _;
    }
